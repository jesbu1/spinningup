...
class MT40(MultiClassMultiTaskEnv, Benchmark):
    ...
    def __init__(self, env_type='train', sample_all=False):
        ...
        all(map(cls_dict.pop, EASY_MODE_CLS_DICT))
        assert len(cls_dict.keys()) == 40
        all(map(args_kwargs.pop, EASY_MODE_ARGS_KWARGS))
        assert len(args_kwargs.keys()) == 40
